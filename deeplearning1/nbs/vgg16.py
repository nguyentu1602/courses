...
class Vgg16():
    ...
    def predict(self, imgs, details=False):
        ...
        """
        # predict probability of each class for each img
        all_preds = self.model.predict(imgs)
        # for each img, get the index of the class with max probability
        idxs = np.argmax(all_preds, axis=1)
        # get the values of the highest probability for each img
        preds = [all_preds[i, idxs[i]] for i in range(len(idxs))]
        # get the label of the class with highest probability for each img
